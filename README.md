...
3.  Migrate your database with `dotnet ef migrations add AddedSomeNewTable` and `dotnet ef database update`
...
- `string email` (required, validated format): the email address of the pet owner
...
- `PetBreed petBreed` (required): Pet breed, based on the `PetBreed` enum.
- `PetColor petColor` (required): Pet color, based on the `PetColor` enum.
...
```bash
curl -H "Content-Type: application/json" -X POST --data '{"name": "Frank Zappa", "email": "frank@zappamusic.org"}' http://localhost:5000/api/petowners/ 2>/dev/null | json_pp
{
   "id" : 76,
   "name" : "Frank Zappa",
   "email" : "frank@zappamusic.org",
   "petCount" : 0
}
```
...
```bash
$ curl -H "Content-Type: application/json" -X POST --data '{"name": "Fido", "petBreed": "bulldog", "petColor": "Black", "petOwnerid": 76}' http://localhost:5000/api/pets 2>/dev/null | json_pp
{
   "id" : 40,
   "petOwner" : {
      "petCount" : 1,
